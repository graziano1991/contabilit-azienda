...
export function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          "fixed inset-y-0 left-0 z-50 flex h-full w-72 max-w-[85vw] shrink-0 flex-col overflow-hidden bg-gradient-to-b from-brand-900 via-[#0d1c3d] to-neutral-950 transition-transform duration-300 ease-snappy",
          "md:static md:z-auto md:w-64 md:max-w-none md:translate-x-0",
          mobileOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"
        )}
      >
        {/* Macchie di luce molto tenui, animate lentamente: danno vita alla
            superficie scura senza distrarre dalla navigazione. */}
        <div className="pointer-events-none absolute -left-16 top-20 h-56 w-56 animate-blob rounded-full bg-accent-500/10 blur-3xl" />
        <div className="pointer-events-none absolute -right-20 bottom-24 h-64 w-64 animate-blob rounded-full bg-brand-400/10 blur-3xl [animation-delay:4s]" />
              ...
              if (!isGroup(entry)) {
                ...
                return (
                  <li key={entry.href}>
                    ...
                    >
                      {active && (
                        <span className="absolute inset-y-1 left-0 w-0.5 rounded-full bg-accent-400 shadow-glow" />
                      )}
                      <Icon
                        className={clsx(
                          "h-4 w-4 shrink-0",
                          ...
                          <li key={item.href}>
                            <Link
                              ...
                              className={clsx(
                                ...
                                active
                                  ? "bg-white/10 font-medium text-accent-300"
