...
export function NavDock({
  ...
  return (
    <>
      {/* Dock flottante desktop: staccato dai bordi (px-4 pt-4), non una
          barra piena larghezza incollata al top come una topbar classica. */}
      <div className="fixed inset-x-0 top-0 z-40 hidden justify-center px-4 pt-4 md:flex">
        <nav className="glass-panel relative flex w-full max-w-6xl items-center gap-1 rounded-2xl px-3 py-2 shadow-ambient">
          <Link
            href="/dashboard"
            className="mr-1 flex shrink-0 items-center gap-2 rounded-xl px-2 py-1.5 transition-opacity hover:opacity-80"
          >
            <Image
              src="/logo.png"
              alt="FinanzaCore"
              ...
              priority
            />
            <span className="hidden whitespace-nowrap text-sm font-semibold text-white lg:inline">
              FinanzaCore
            </span>
          </Link>
              ...
              if (!isGroup(entry)) {
                ...
                return (
                  <Link
                    ...
                    title={entry.label}
                    className={clsx(
                      "flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-xl px-2.5 py-2 text-sm font-medium transition-all duration-150",
                      active
